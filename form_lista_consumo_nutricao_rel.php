...
<?php
    ...
    // Tipo de relatório por apenas 1 lote (pode ser por periodo tambem)
        ...
        // Só soma uma vez por data (ver comentário na inicialização de
        // $data_dias_contabilizados).
        if ($data_bruta_impressa !== $data_dias_contabilizados) {
            $total_dias += ($dias_consumo_anterior==0) ? $quantidade_dias[0] : $dias_consumo_anterior;
            $data_dias_contabilizados = $data_bruta_impressa;
        }
    ...
    function monta_produto($conector, $local_filtro, $id_lote, $data_inicial, $data_final, $wproduto=''){
        ...
        $tbl_nutricao = mysqli_query($conector, "SELECT tbl_produto_descricao from tbl_nutricao
            INNER JOIN tbl_produto
                    ON tbl_nutricao_codigo_produto = tbl_produto_codigo_id
