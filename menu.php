...
// FUNÇÃO BLINDADA: Remove espaços e TABs no início de cada linha via REGEX
/*function limparDescricao($texto) {
    // 1. MANTÉM sua regra original: Remove caracteres invisíveis (BOM, espaços extras)
    $texto = trim($texto, "\x00..\x1F"); 
    
    // 2. MANTÉM sua regra original: Remove espaços e tabs no início de cada linha
    $texto = preg_replace('/^[ \t]+/m', '', $texto);
    
    // 3. NOVIDADE: Como o seu banco usa ";" para separar os tópicos, 
    // vamos trocar o ";" por ";" seguido de uma quebra de linha real.
    $texto = str_replace(';', '; ' . PHP_EOL, $texto);
    
    // 4. MANTÉM o ltrim e ADICIONA o nl2br para o HTML entender as quebras
    return nl2br(ltrim($texto));
}*/
...
<head>
    ...
    <style type="text/css">
        ...
        /* ESTILOS PARA A TELA VERSAO */
        /* 1. Mantém a largura padrão do sistema e limita a altura com scroll */
        #versao .modal-body {
            max-height: 400px; 
            overflow-y: auto;
            padding: 20px;
        ...
        /* 2. Estilos da Linha do Tempo (Timeline) */
        #versao .timeline-wrapper {
            position: relative;
            padding-left: 35px;
            margin-top: 15px;
            display: block;
        ...
        /* 3. Cards do Histórico */
        #versao .card-historico {
            position: relative;
        ...
        /* 4. Destaque da Versão Atual (AGORA EM AZUL #72b8f1) */
        #versao .card-historico.atual { 
            border-color: #72b8f1; 
            background-color: #f0f7ff !important; /* Um azul bem clarinho para o fundo */
        }

        /* 5. A Bolinha da Linha do Tempo */
        #versao .dot {
            position: absolute;
            left: -28px;
            top: 18px;
        ...
        /* Bolinha azul para a versão atual */
        #versao .card-historico.atual .dot {
            background: #72b8f1;
            border-color: #72b8f1;
        }
                ...
                <div class="modal fade" id="versao" tabindex="-1" role="dialog" aria-labelledby="exampleModalLongTitle" aria-hidden="true">
                    <div class="modal-dialog" role="document">
                        <div class="modal-content">
                            ...
                            <div class="modal-header">
                                <button type="button" class="close" data-dismiss="modal" aria-hidden="true">&times;</button>
                                <h3 class="modal-title" id="myModalLabel" style="color:#0a5074">Notas da Versão</h3>
                            </div>

                            <div class="modal-body nao-pode-selecionar">
                                
                                <div id="v-resumo" style="text-align: center;">
                                    <img src="img/boi_virtual_preto.png" alt="Boi Virtual" style="width: 80px; margin-bottom: 10px;">
                                    <h4 style="margin: 0; color: #333; font-weight: bold;">Versão <?php echo $atual['tbl_versao_numero']; ?></h4>
                                    <p style="color: #888; font-size: 0.85em;"><?php echo $data_atual_br; ?></p>
                                    
                                    <div style="text-align: left; background: #f9f9f9; padding: 15px; border-radius: 8px; border: 1px solid #eee; margin-top: 15px;">
                                        <strong style="font-size: 0.95em; color: #555;">Novidades:</strong>
                                        <div class="texto-descricao"><?php echo tratarDescricaoVersao($atual['tbl_versao_descricao']); ?></div>
                                    </div>

                                    <div style="margin-top: 20px;">
                                        <a href="javascript:void(0)" class="btn-link-historico" onclick="alternarVisaoVersao('historico')">
                                            <i class="fas fa-history"></i> Ver histórico completo
                                        </a>
                                    </div>
                                </div>

                                <div id="v-historico" style="display: none;">
                                    <div class="timeline-wrapper">
                                        <?php foreach ($versoes as $index => $v): 
                                            $data_v = date('d/m/Y', strtotime($v['tbl_versao_data']));
                                        ?>
                                            <div class="card-historico <?php echo $index === 0 ? 'atual' : ''; ?>">
                                                <div class="dot"></div>
                                                <strong style="font-size: 0.9em;">Versão <?php echo $v['tbl_versao_numero']; ?></strong>
                                                <small style="color:#999; margin-left: 10px;"><?php echo $data_v; ?></small>
                                                <div class="texto-descricao" style="border-top: 1px solid #f2f2f2; padding-top: 8px; margin-top: 8px;">
                                                    <?php echo tratarDescricaoVersao($v['tbl_versao_descricao']); ?>
                                                    <?php //echo limparDescricao($v['tbl_versao_descricao']); ?>
                                                </div>
                                            </div>
                                        <?php endforeach; ?>
                                    </div>
                                    ...
                                    <div style="text-align: center; margin-top: 15px;">
                                        <a href="javascript:void(0)" class="btn-link-historico" style="color: #666;" onclick="alternarVisaoVersao('resumo')">
...
<script>
function alternarVisaoVersao(tipo) {
    if (tipo === 'historico') {
        $('#v-resumo').hide();
        $('#v-historico').fadeIn();
        $('#versao #titulo-modal-texto').text('Histórico de Alterações');
        
        // Faz o scroll subir para o topo ao abrir o histórico
        $('#versao .modal-body').scrollTop(0);
        
    } else {
        $('#v-historico').hide();
        $('#v-resumo').fadeIn();
        // Volta o título para o padrão da Versão Atual
        $('#versao #titulo-modal-texto').text('Versão <?php echo $atual['tbl_versao_numero']; ?> - <?php echo $data_atual_br; ?>');
    }
}

// Resetar para o resumo toda vez que o modal for fechado
$('#versao').on('hidden.bs.modal', function () {
    $('#v-historico').hide();
    $('#v-resumo').show();
});
</script>
