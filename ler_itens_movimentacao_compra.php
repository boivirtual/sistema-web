...
if ($num_rows!=0){
    while ($fila = mysqli_fetch_object($rs)){
    	$idade_compra = $fila->tbl_ite_idade_meses_compra;
		$sexo = $fila->tbl_ite_movimentacao_sexo;
		$cabecas = $fila->tbl_ite_qtd_categoria_compra;
		$peso = 0;

		$inicio=date("Y-m-d");
		$data_termino = new DateTime($inicio);
		$data_termino->sub(new DateInterval('P'.$idade_compra.'M'));
		...
		if ($num_rows_cat!=0) {
			while ($reg_categoria = mysqli_fetch_object($categoria)) {
			    ...
			    if ($idade >= $idade_de && $idade <= $idade_ate) {
			    	...
			    	$total_categoria[$id_categoria]+=$cabecas;

			        if ($sexo=="F") {
		        		$total_femea[$id_categoria]+=$cabecas;
		        		$peso_femea[$id_categoria]+=$peso;
			        }
			        else {
       					$total_macho[$id_categoria]+=$cabecas;
