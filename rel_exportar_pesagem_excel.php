<?php
    ...
    $num_natimorto_de = $d->num_natimorto_de;
    $num_natimorto_ate = $d->num_natimorto_ate;
    $previsao_parto_de = $d->previsao_parto_de;
    $previsao_parto_ate = $d->previsao_parto_ate;
    $data_paricao_de = $d->data_paricao_de;
    $data_paricao_ate = $d->data_paricao_ate;
    $filtro_reproducao = $d->filtro_reproducao;
    $filtro_num_parto = $d->filtro_num_parto;
    $filtro_num_aborto = $d->filtro_num_aborto;
    $filtro_num_natimorto = $d->filtro_num_natimorto;
    $filtro_previsao_parto = $d->filtro_previsao_parto;
    $filtro_data_paricao = $d->filtro_data_paricao;
    $filtro_vacas_paridas = $d->filtro_vacas_paridas;
    $filtro_vacas_solteiras = $d->filtro_vacas_solteiras;
    $filtro_vacas_prenhas = $d->filtro_vacas_prenhas;
    $filtro_descarte = $d->filtro_descarte;
    $filtro_positivas = $d->filtro_positivas;
    $filtro_negativas = $d->filtro_negativas;
    $filtro_monta_natural = $d->filtro_monta_natural;
    $estacao_filtro = $d->filtro_estacao;

    $local_filtro = $d->local;
    $epoca = $d->epoca;
    $origem_filtro = $d->origem;
    $desc_filtro = $d->desc_filtro;
    $desc_filtro_gravar = $d->desc_filtro;
    $sexo_filtro = $d->sexo;
    $raca_filtro = $d->codigos_racas;
    $categoria_filtro = $d->categoria;
    $pai_filtro = $d->codigos_pais;
    $mae_filtro = $d->codigos_maes;
    ...
    if (isset($estacao_filtro) && is_array($estacao_filtro) && !empty($estacao_filtro)) {
        
        $estacoes = $estacao_filtro;
    ...
    if ($data_nasc_inicial_filtro==0 && $data_nasc_final_filtro==0){
        $wdata_nasc = '';
    }
    else {
    ...
    if ($num_rows != 0) {
        ...
        $descricao_local = $reg_local->tbl_pessoa_nome;
    } else {
        $descricao_local = 'Não Informado';
    }
    ...
    while ($reg_animal = mysqli_fetch_object($tbl_animais)){
        ...
        //if ($wcategoria=="") {
            /*$animais_json_string = json_encode($wcategoria);
            header('Content-type: application/json');
            echo json_encode(array('error' => true, 'message' => 'Passei aqui ' .
                $animais_json_string));
            exit;*/
        //}
    ...
    if (empty($animais)) {
        header('Content-type: application/json');
        echo json_encode(array('error' => true, 'message' => 'Não existem animais para listar!'));
        exit;
    }
            ...
            if ($filtro_negativas=='S' && $filtro_positivas=='S') {
                if ($filtro_vacas_paridas=='N' && $filtro_vacas_solteiras=='N' && 
                    ...
                    if ($animal['filtroNumPartos']=='S' && 
                        ...
                        $animais_listados++;
                        $numero_item = listar($conector, $animal, $nascimento_edi, $descarte, $numero_item, $numero_pesagem, $data_pesagem);
                    }
                }
    ...
    if ($numero_item == 0) {
        ...
        header('Content-type: application/json');
        echo json_encode(array('error' => false, 'message' => '', 'num_pesagem' => $numero_pesagem, 'desc_filtro' => $desc_filtro, 'desc_local' => $descricao_local, 'desc_epoca' => $desc_epoca));
    }


    function listar($conector, $animal, $nascimentoEdi, $descarte, $numero_item, $numero_pesagem, $data_pesagem)
    {
        $numero_item++;
        $codigo_id_animal = $animal['codigoAnimal'];

        if ($animal['codigoAlfa'] != '') {
            $id_edi = $animal['codigoAlfa'] . '-' . $animal['codigoNumerico'];
        } else {
            $id_edi = $animal['codigoNumerico'];
        }
        ...
        $desc_raca = $animal['descricaoRaca'];
        $desc_pelagem = $animal['descricaoPelagem'];
        $desc_mae = $animal['codigoMaeAlfaNumerico'];
        $codigo_categoria = $animal['codigoCategoria'];
