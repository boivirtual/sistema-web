...
while ($reg_animal = mysqli_fetch_object($tbl_animais)) {
    ...
    if ($tipo_cobertura=='I') { // Tipo Cobertura IATF
    ...
    else { // Tipo Cobertura Monta

        $filtro_com_previsao = $_POST["filtro_com_previsao"];
        $filtro_sem_previsao = $_POST["filtro_sem_previsao"];

        if ($filtro_com_previsao=='S' && $filtro_sem_previsao=='S') {
            $sql =  "SELECT * FROM tbl_item_cobertura 
                INNER JOIN tbl_cobertura
                        ON tbl_ite_cobertura_numero_id = tbl_cobertura_id
                WHERE tbl_ite_cobertura_codigo_id_animal = '$codigo_id' AND 
                    ...
                    (tbl_ite_cobertura_previsao_parto>='$previsao_parto_de'
                    AND 
                    tbl_ite_cobertura_previsao_parto<='$previsao_parto_ate'
                    OR tbl_ite_cobertura_previsao_parto is null) AND
                    tbl_ite_cobertura_nascido is null
                ORDER BY tbl_cobertura_id DESC LIMIT 1"; 
        }
        else if ($filtro_com_previsao=='S' && $filtro_sem_previsao=='') {
            $sql =  "SELECT * FROM tbl_item_cobertura 
                INNER JOIN tbl_cobertura
                        ON tbl_ite_cobertura_numero_id = tbl_cobertura_id
                WHERE tbl_ite_cobertura_codigo_id_animal = '$codigo_id' AND 
        ...
        else if ($filtro_com_previsao=='' && $filtro_sem_previsao=='S') {
            $sql =  "SELECT * FROM tbl_item_cobertura 
                INNER JOIN tbl_cobertura
                        ON tbl_ite_cobertura_numero_id = tbl_cobertura_id
                WHERE tbl_ite_cobertura_codigo_id_animal = '$codigo_id' AND 
        ...
        if ($tem_thead == 'N'){
            ...
            echo "<input type='hidden' id='local_id_monta' value='$codigo_local'>";
            echo "<input type='hidden' id='comPrevisao' value='$filtro_com_previsao'>";
            echo "<input type='hidden' id='semPrevisao' value='$filtro_sem_previsao'>";

            echo '<div class="row">
                  <div class="form-group col-md-12">'; 

            if ($filtro_com_previsao=='S') {
                echo '<label class="checkbox-inline">
                      <input type="checkbox" id="comprevisao" name="com_sem_previsao" value="C" onclick="ver_previsao()"
                      checked> Com Previsão 
                      </label>';
            }
            else {
                echo '<label class="checkbox-inline">
                      <input type="checkbox" id="comprevisao" name="com_sem_previsao" value="C" onclick="ver_previsao()"> Com Previsão 
                      </label>';
            }

            if ($filtro_sem_previsao=='S') {
                echo '<label class="checkbox-inline">
                      <input type="checkbox" id="semprevisao" name="com_sem_previsao" value="S" onclick="ver_previsao()"
                      checked> Sem Previsão 
                      </label>';
            }
            else {
            echo '<label class="checkbox-inline">
                  <input type="checkbox" id="semprevisao" name="com_sem_previsao" value="S" onclick="ver_previsao()"> Sem Previsão 
                  </label>';
            }
        ...
        if ($num_rows_coberturas > 0) {
            while($reg_cobertura = mysqli_fetch_object($tbl_cobertura)){
                ...
                echo "<td width='12%'>".$desc_raca."</td>";
                echo "<td align='center' width='12%'>".$idade."</td>";
                echo "<td align='center' width='10%'>".$numero_partos."</td>";
                echo "<td align='center' width='10%'>".$num_abortos."</td>";

                echo "<td align='center' width='12%'><input type='date' name='data_prenhes$cobertura_id' 
                    id='data_prenhes$cobertura_id' class='form-control
                    input-sm' value='$data_prenhes' 
                    onkeypress='return desabilita_enter(this, event)' 
                    onblur='calcular_data_previsao(this.id, this.value)'
                    onchange='prenhes_limpa_positiva(this.id, 
                    this.value)'></td>";

                echo "<td align='center' width='16%'><input type='date' name='data_previsao$cobertura_id' id='data_previsao$cobertura_id' class='form-control input-sm' value='$data_previsao_parto' onkeypress='return desabilita_enter(this, event)' onblur='calcular_data_prenhes(this.id, this.value)' onchange='previsao_limpa_positiva(this.id, this.value)'></td>";
