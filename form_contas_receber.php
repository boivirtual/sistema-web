...
<head>
    ...
    <style type="text/css">
        ...
        table#tabela_contas_receber { border-top: none !important; }
        table#tabela_contas_receber thead > tr:first-child > th,
        table#tabela_contas_receber thead > tr:first-child > td { border-top: none !important; }
...
<body>
    ...
    if (isset($_GET['reset']) && $_GET['reset'] == '1') {
        $_SESSION['data_inicio_ctr']    = '';
        $_SESSION['data_fim_ctr']       = '';
        $_SESSION['periodo_label_ctr']  = '';
    ...
    $stored_inicio = $_SESSION['data_inicio_ctr']   ?? null;
    $stored_fim    = $_SESSION['data_fim_ctr']      ?? null;
    $stored_label  = $_SESSION['periodo_label_ctr'] ?? '';

    // Sem sessão: usa mês inteiro. Se início==fim mas não veio de um filtro explícito
    // (label vazio = padrão antigo que salvava só o dia atual), também usa mês inteiro.
    // Quando há label salvo (ex.: "Hoje" ou um Período Customizado de um único dia),
    // respeita o intervalo salvo mesmo com início==fim.
    if (!$stored_inicio || !$stored_fim || ($stored_inicio === $stored_fim && !$stored_label)) {
    ...
    $local              = $_SESSION['codigo_local_ctr']   ?? '';
    $contas             = $_SESSION['codigo_conta_ctr']   ?? '';
        ...
        <section id="main-content">
            <section class="wrapper" style="margin-left: 5px;">
                <span class="caminho-programa">Gestão Administrativa <i class="fa fa-angle-right seta-direita"></i>
                    <span class="titulo">Contas a Receber</span></span>
                ...
                <div class="row">
                    <div class="col-lg-12">
                        ...
                        <div class="row col-md-12" id="consulta_contas">
                            <form method="GET" action="form_contas_receber.php" enctype="multipart/form-data" id="form_consulta_contas">
                                    ...
                                    <div class="tab-pane active">
                                        ...
                                        <fieldset class="scheduler-border" id="dados_consulta">
                                            ...
                                            <div class="row">
                                                ...
                                                <div class="form-group col-md-3">
                                                    <label for="razao_nome" class="control-label">Cliente</label>
                                                ...
                                                <div class="form-group col-md-2">
                                                    ...
                                                    <div style="display: flex; gap: 6px;">
                                                        ...
                                                        <button type="button" id="btn_consultar_filtro" class="form-control btn btn-primary" onclick="consultar_ctr()" style="display:none;">
                                                            Consultar
                                                        </button>
                                                    </div>
                                                </div>
                                            ...
                                            <div class="row filtros" hidden>
                                                <div class="col-md-12">
                                                    ...
                                                    <a href="#" style="font-size: 0.9em; font-weight: 500; text-align: right; color: #128cb8; float: right;" onclick="mais_relatorios()" data-toggle='tooltip' data-placement='top' title="Análise de Recebimentos" class="pull-right"><i class="fa fa-plus"></i> Relatórios</a>
                                                </div>
                                            </div>
                                        </fieldset>
...
<script>
    ...
    $(document).ready(function() {
        ...
        if (dataIniSessao && dataFimSessao) {
            ...
            if (ehMesUnico) {
                ...
                if (labelSalvo === 'Este Mês' || labelSalvo === 'Mês Passado') {
                    ...
                    ctrFiltroModal.radio = (labelSalvo === 'Este Mês') ? 'mes' : 'mes_passado';
                }
            } else {
                ...
                if (labelSalvo) {
                    ...
                    if (radioKey) {
                        ctrFiltroModal.radio = radioKey;
                    } else {
                        ctrFiltroModal.dataInicio = dataIniSessao;
                        ctrFiltroModal.dataFim    = dataFimSessao;
                    }
                } else {
        ...
        // Restaura selectpickers ANTES de consultar_ctr() porque $(window).load roda depois do document.ready
        ...
        var _exCli = $('#exibe_cliente').val();
        if (_exCli) {
            _exCli.split(',').filter(function(v){ return v !== ''; }).forEach(function(v) {
        ...
        // Se há filtro de Conta Contábil, o $(window).load carrega as opções e chama consultar_ctr()
        ...
        // Exibir botão Consultar e ocultar listagem ao alterar Local ou Cliente
        $('#codigo_fazenda, #razao_nome').on('changed.bs.select', function() {
            $('#btn_consultar_filtro').show();
            $('#lista_contas_receber').hide();
            atualizarLinkLimparFiltros();
        });
        ...
        $('#modal_seletor_periodo').on('shown.bs.modal', function() {
            ...
            if (ctrFiltroModal.radio) {
                $('input[name="periodo_rapido"][value="' + ctrFiltroModal.radio + '"]').prop('checked', true);
            }
            // Restaura datas customizadas
            if (ctrFiltroModal.dataInicio) {
                $('#data_inicio_custom').val(ctrFiltroModal.dataInicio);
            }
            if (ctrFiltroModal.dataFim) {
                $('#data_fim_custom').val(ctrFiltroModal.dataFim);
            }
        });
    ...
    var ctrFiltroModal = { radio: null, dataInicio: null, dataFim: null };
    ...
    function atualizarLinkLimparFiltros() {
        ...
        var temPeriodo  = ctrFiltroModal.radio !== null || ctrFiltroModal.dataInicio !== null;
        var temLocal    = $('#codigo_fazenda').val() && $('#codigo_fazenda').val().length > 0;
        var temCliente  = $('#razao_nome').val() && $('#razao_nome').val().length > 0;

        if (temCC || temConta || temPeriodo || temLocal || temCliente) {
    ...
    function limparFiltrosModal() {
        ...
        ctrFiltroModal = { radio: null, dataInicio: null, dataFim: null };
        $('input[name="periodo_rapido"]').prop('checked', false);
    ...
    function navegarMesAnterior() {
        $('#periodo_label').val('');
        ctrFiltroModal.radio = null;
        ctrFiltroModal.dataInicio = null;
        ctrFiltroModal.dataFim = null;
    ...
    function aplicarSelecaoPeriodo() {
        ...
        ctrFiltroModal.radio      = radioSelecionado || null;
        ctrFiltroModal.dataInicio = radioSelecionado ? null : ($('#data_inicio_custom').val() || null);
        ctrFiltroModal.dataFim    = radioSelecionado ? null : ($('#data_fim_custom').val()    || null);
