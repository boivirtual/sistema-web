...
$(document).ready(function(){
    ...
    $(".tipo_rel").click(function(){
        ...
        aplicar_filtros(); // exibe filtros
    });

    $("#macho").click(function(){
        $('#macho_filtro').prop('checked', $(this).is(':checked'));
    });

    $("#macho_filtro").click(function(){
        $('#macho').prop('checked', $(this).is(':checked'));
    });
    ...
    $('#macho_filtro').on('change', function() {
        var macho = $('#macho_filtro');
        var femea = $('#femea_filtro');
    ...
    $("#femea").click(function(){
        $('#femea_filtro').prop('checked', $(this).is(':checked'));
    });

    $("#femea_filtro").click(function(){
        $('#femea').prop('checked', $(this).is(':checked'));
    });
    ...
    $('#femea_filtro').on('change', function() {
        var femea = $('#femea_filtro');
        var macho = $('#macho_filtro');
    ...
    $('#codigo_categoria_filtro').change(function(){
        ...
        $('#codigo_categoria_filtro_filtro').selectpicker('val', categoria);
    ...
    $('#codigo_categoria_filtro_filtro').change(function(){
        ...
        var categoria = $("#codigo_categoria_filtro_filtro").val();
        $('#codigo_categoria_filtro').selectpicker('val', categoria);
    });
    ...
    $('#positivo').click(function(){
        ...
        if ($("#positivo").is(":checked") == true){
            ...
            document.getElementById("codigo_estacao_filtro").style.borderColor = "#2556db";
...
function filtros_gmd() {
    var data_inicial = $("#data_inicial").val();
    var data_final = $("#data_final").val();

    if (!data_inicial || !data_final) {
        $("#mensagem_erro").modal();
        $("#mensagem_erro .modal-body").html('Selecione as Datas Inicial e Final.');
        return;
    }
...
function abrir_filtro_reproducao() {
    ...
    if (femea.is(":checked") && macho.is(":checked")){
...
function abrir_filtro_reproducao_continuar() {
    ...
    $("#macho_filtro").prop("checked", false);

    const selectElement = document.getElementById('codigo_categoria_filtro_filtro');
...
function limpar_filtros(){
    ...
    $("#macho_filtro").prop("checked", true);
    $("#femea").prop("checked", true);
    $("#femea_filtro").prop("checked", true);
...
function aplicar_filtros() {
    ...
    if (codigo_numerico!='') {
        ...
        var data1 = new Date(data_inicial); 
        var data2 = new Date(new Date(data_final));
        var qtd_meses = (data2.getFullYear() - data1.getFullYear())*12 + (data2.getMonth() - data1.getMonth());
        qtd_meses++;

        if (qtd_meses>12) {
            $("#mensagem_erro").modal();
            $("#mensagem_erro .modal-body").html('Selecione no máximo 12 meses.');
            return;
        }

        var data_ini = data_inicial.split("-");
        var data_fim = data_final.split("-");
        var periodo_filtro = 'Período da Pesagem: ' + data_ini[1]+'/'+data_ini[0] + ' até ' + data_fim[1]+'/'+data_fim[0]+'->';
        ...
        if (positivo=='S' || negativo=='S') {
            if (filtro_estacao=='' && monta_natural=='') {
                $("#mensagem_erro_filtro").modal();
                $("#mensagem_erro_filtro .modal-body").html('Para Filtro Positivas ou Negativas, selecione a Estação de Monta ou Monta Natural!');
                document.getElementById("codigo_estacao_filtro").focus();
                document.getElementById("codigo_estacao_filtro").style.borderColor = "red";
                return;
            }
        }
...
function exibe_filtro(){
    ...
    if (codigo_numerico!='') {
        ...
        var descricao_filtro = 'Filtros: '+periodo_filtro+
            local_filtro+
            categoria_filtro+
            filtro_ativo+
            filtro_vendido+
    ...
    $("#filtro_aplicado").html(descricao_filtro);
...
function veriricar_tipo_relatorio(opcao) {
    ...
    if (codigo_numerico!='') {
        $.post("ler_animal_aplicar_filtro.php", {id_animal:codigo_numerico}, function(valor){
            if (valor==0) {
                $("#mensagem_erro_animal_filtro").modal();
                $("#codigo_number_filtro").val('');
                document.getElementById("codigo_number_filtro").focus();
...
function listar_gmd(opcao){
    ...
    var num_aborto_ate =$("#num_aborto_ate_filtro").val();
    var filtro_estacao = $("#codigo_estacao_filtro").val();
    var descricao_filtro = $("#filtro_aplicado").html();
    ...
    if (opcao=='1') {
        if (tipo_rel=='G') {
            location.href='form_lista_gmd_individual_rel.php?descricao_filtro=' + descricao_filtro + 
            '&tipo_rel=' + tipo_rel + 
            '&local=' + array_local + 
            '&origem=' + array_origem + 
            ...
            location.href='form_lista_gmd_categoria_rel.php?descricao_filtro=' + descricao_filtro + 
            '&tipo_rel=' + tipo_rel + 
            '&local=' + array_local + 
            '&origem=' + array_origem + 
            '&categoria=' + array_categoria + 
        ...
        if (tipo_rel=='G') {
            location.href='rel_gmd_individual_excel.php?descricao_filtro=' + descricao_filtro + 
            '&tipo_rel=' + tipo_rel + 
            '&local=' + array_local + 
            '&origem=' + array_origem + 
            '&categoria=' + array_categoria + 
            ...
            location.href='rel_gmd_categoria_excel.php?descricao_filtro=' + descricao_filtro + 
            '&tipo_rel=' + tipo_rel + 
            '&local=' + array_local + 
            '&origem=' + array_origem + 
            '&categoria=' + array_categoria + 
...
function lista_gmd_excel(){
    ...
    var situacao_vendido = $("#vendido").val();
    var situacao_morte = $("#morte").val();
    var situacao_outra = $("#outro").val();
    var data_inicial = $("#data_inicial").val();
    var data_final = $("#data_final").val();
