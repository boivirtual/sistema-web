...
$primeiro_ctp_id = isset($_POST['primeiro_ctp_id']) ? intval($_POST['primeiro_ctp_id']) : 0;
$json            = isset($_POST['rateio_json'])      ? trim($_POST['rateio_json'])       : '';

if ($primeiro_ctp_id <= 0) {
...
// Remove rateio atual
if (!mysqli_query($conector, "DELETE FROM tbl_ctp_rateio WHERE rc_ctp_id = '$primeiro_ctp_id'")) {
    echo json_encode(['error' => true, 'message' => 'Erro ao remover rateio antigo: ' . mysqli_error($conector)]);
    exit;
}

// Regrava com a mesma lógica de salvar_rateio() em gravar_contas_pagar.php
