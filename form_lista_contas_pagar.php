            ...
            <tbody>
                <?php
                    ...
                    if ($criterio=='') {
                        ...
                        // Parcelamento sem número de documento (nem repetição): o anexo precisa
                        // ser vinculado a todas as parcelas do mesmo fornecedor + instante de
                        // inclusão idêntico (todas as parcelas de um lançamento são gravadas no
                        // mesmo instante) — senão só aparecia na parcela onde foi anexado.
                        $fornec_instante_com_anexo = [];
                        ...
                        if ($rs_anx_instante) {
                            while ($ra = mysqli_fetch_object($rs_anx_instante)) {
                                $fornec_instante_com_anexo[$ra->ctp_codigo_fornecedor . '|' . $ra->ctp_incluido_em] = true;
                            }
                        }
                        ...
                        while ($registro_ctp = mysqli_fetch_object($rs)){
                            ...
                            // Ícone de anexo — aparece se o documento (ou o grupo de repetição/parcelamento) tem
                            // arquivos/links em tbl_ctp_anexos. Checagem em cascata: uma baixa individual pode
                            // preencher o Número do Documento só desta linha, sem que o anexo (salvo em outra
                            // parcela/ocorrência do mesmo lançamento) a acompanhe — por isso, se a estratégia mais
                            // específica não achar nada, tenta a próxima em vez de desistir.
                            $grupo_repeticao = $registro_ctp->ctp_grupo_repeticao;
                            $tem_anexo = false;
                            if ($eh_repeticao) {
                                $tem_anexo = isset($grupos_com_anexo[$grupo_repeticao]);
                            }
                            if (!$tem_anexo && !empty($numero_doc)) {
