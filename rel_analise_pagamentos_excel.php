...
if ($codigo_fazenda!='') {
    $wfazendas = " AND (ctp_codigo_fazenda IN($fazendas) OR " . condicao_rateio_ou_grupo('ctp_codigo_fazenda', 'rc_codigo_local', $fazendas) . ")";
}
...
if ($codigo_cc!='') {
    $wcc = " AND (ctp_codigo_centro_custos IN($centro_custo) OR " . condicao_rateio_ou_grupo('ctp_codigo_centro_custos', 'rc_codigo_cc', $centro_custo) . ")";
}
...
$linha=4;
    ...
    */
    ...
    if ($tipo_data=="E"){
        $contas_pag = "SELECT * FROM contas_pagar
	                                    ...
	                                    for ($k=0; $k < count($array_contas); $k++) { 
	                                        if ($k==0){
												...
												$spreadsheet->setActiveSheetIndex(0)
											    	...
											    	->setCellValue("C".$linha,"Fornecedor")
...
function ler_notas($conector, $data_sistema,$tipo_data,$data_inicial,$data_final,$conta_inicio,$wfazendas,$wfornecedor,$wcc,$fazendas_ids='',$cc_ids=''){
    ...
    while ($registro_contas_pag = mysqli_fetch_object($contas_pag)){
        ...
        //$razao = utf8_encode(substr($registro_contas_pag->ctp_nome_fornecedor, 0,38));
