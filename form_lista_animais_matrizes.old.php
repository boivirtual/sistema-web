...
<?php
    ...
    $sql = mysqli_query($conector, "SELECT * FROM tbl_animais
        WHERE tbl_animal_codigo_id=1237 or tbl_animal_codigo_id=1263"); 
    ...
    print_r('Registros encontrados: ' . $num_row . '</br>');
    ...
    if ($num_row!=0) {
        while ($reg_animal = mysqli_fetch_object($sql)){
            ...
            print_r('Id Animal: ' . $codigo_numerico . '</br>');
            ...
            if ($numero_rows_partos!=0) {
                ...
                print_r('Dias parto: ' . $dias_parto . '</br>');
            ...
            if ($num_natimorto!=0) {
                ...
                print_r('Dias natmorto: ' . $dias_parto . '</br>');
            ...
            if ($num_rows_prenhe==0 && $idade>=12 && $femea_selecionada=='' && $ultimo_peso>=$peso_acima) {
                ...
                if ($sem_filtros=='S' && $imp_esse_sem_filtro=='S') {
                    ...
                    $animais_listados = ImprimirFemea($conector, $animais_listados, $codigo_id, $codigo_alfa, $codigo_numerico, $data_nascimento, $ultimo_peso, $codigo_raca, $descarte, $local, $id_parametro_estacao, $idade_ano_mes);
                }
...
// IMPRIME A FEMEA
function ImprimirFemea($conector, $animais_listados, $codigo_id, $codigo_alfa, $codigo_numerico, $data_nascimento, $ultimo_peso, $codigo_raca, $descarte, $local, $id_parametro_estacao, $idade_ano_mes) {
    ...
    if ($tem_natimorto=='S') {
        echo '<td width="9%">'.$ultimo_parto_edi.'&nbsp;&nbsp;<i class="icon_info_alt" data-toggle="tooltip" data-placement="right" title="Considerado aqui a data do Natimorto." style="color: red;"></i></td>';
    }
    else {
