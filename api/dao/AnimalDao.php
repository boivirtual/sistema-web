...
/*    public function getAnimalById($id, $local){
        $sql = "SELECT * 
        FROM tbl_animais
        LEFT JOIN tabela_racas ON tbl_animal_codigo_raca = tab_codigo_raca
        LEFT JOIN tabela_pelagens ON tbl_animal_codigo_pelagem = tab_codigo_pelagem
        LEFT JOIN tbl_pessoa ON tbl_animal_codigo_fazenda = tbl_pessoa_id
        WHERE tbl_animal_lixeira = 0 AND tbl_animal_ativo = 'S' 
        AND tbl_animal_codigo_fazenda = '$local' AND tbl_animal_codigo_id = $id";
...
/*    public function getLoteAbertoPorAnimal($idAnimal) {
        // SQL ajustado com seus nomes de tabelas e campos
        $sql = "SELECT p.tbl_pesagem_lote 
                FROM tbl_item_pesagem i
                JOIN tbl_pesagem p ON i.tbl_ite_pesagem_numero_id = p.tbl_pesagem_id
                WHERE i.tbl_ite_pesagem_codigo_id_animal = $idAnimal 
                AND p.tbl_pesagem_finalizada = 'N' 
