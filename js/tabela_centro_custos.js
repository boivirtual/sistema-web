...
$(document).ready(function(){
    $('#tabela_centro_custos').DataTable({
        responsive: true,
        paging: false,
        ordering: true,
        info: true,
    ...
    $('.gravar').on('click', function() {
        var formID = document.getElementById("gravar_cc");
...
$('#modal_excluir').on('show.bs.modal', function (event) {
    ...
    var modal = $(this)
    if (tipo_gravacao==3){
        modal.find('.modal-title').text('Centro de Custos - Restaurar da lixeira ')
    }
    else if (tipo_gravacao==2){
        modal.find('.modal-title').text('Centro de Custos - Enviar para lixeira ')
    }
    //modal.find('.modal-title').text('Edição da raça - código: ' + recipient)
   // modal.find('.modal-title').text('Edição da raça')
    modal.find('#codigo_tipo').val(recipient)
