...
$(document).ready(function(){
    $('.gravar').on('click', function() {
        var formID = document.getElementById("gravar_raca");
...
$('#modal_editar').on('show.bs.modal', function (event) {
    ...
    modal.find('#codigo_raca').val(recipient)
    modal.find('#descricao_raca').val(recipientnome)
})
...
$('#modal_excluir').on('show.bs.modal', function (event) {
    ...
    var modal = $(this)
    if (tipo_gravacao==3){
        modal.find('.modal-title').text('Racas de Animais - Remover da lixeira ')
    }
    else if (tipo_gravacao==2){
        modal.find('.modal-title').text('Racas de Animais - Enviar para lixeira ')
    }
    //modal.find('.modal-title').text('Edição da raça - código: ' + recipient)
   // modal.find('.modal-title').text('Edição da raça')
    modal.find('#codigo_raca').val(recipient)
