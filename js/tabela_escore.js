...
function listar_escore(){
    ...
    $.post("form_lista_escore.php", {local:local},
        function(valor){ $("div[id=lista_escore]").html(valor); 
    });
}

$(document).ready(function(){
    $('#tabela_escore').DataTable({
        "responsive": true,
        "paging":   false,
        "ordering": false,
        "info":     true,
...
function voltar_inclusao(){
    location.href='form_tabela_escore.php';
}
...
function incluir_novo() {
    ...
    $('#modal_incluir .modal-title').html('Escore - Incluir');
...
function editar_escore(array_registro) {
    $array_conta = array_registro.split('|');
    $("#codigo_conta").val($array_conta[0]);
    $("#descricao").val($array_conta[1]);
    $("#aparencia").val($array_conta[2]);

    $("#tipo_gravacao").val(1);

    $('#modal_incluir .modal-title').html('Escore - Editar');
...
function enviar_lixeira(array_registro, opcao) {
    ...
    if (opcao==2) {
        $('#modal_incluir .modal-title').html('Escore - Enviar para Lixeira');
        $(".confirma_gravar").html('Enviar para Lixeira').removeClass('btn-primary').addClass('btn-danger');
    }
    else {
        $('#modal_incluir .modal-title').html('Escore - Remover da Lixeira');
