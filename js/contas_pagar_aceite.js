...
*/
    ...
    $('#lista_contas_pagar').load('form_lista_contas_pagar_aceite.php?data_inicial=' + data_inicial +
...
function baixar_conta_selecionada() {
   ...
   // }
    ...
    //if (confirm("Confirma a baixa das contas selecionadas?")) {
...
function somar_total_para_baixar() {
    ...
    $.post("somar_contas_pagar_selecionadas_aceite.php", {grupo_contas: grupo_contas}, function (get_retorno) {
        if (get_retorno != '') {
            var total_selecionado =formatMoney(get_retorno);
            $("#total_selecionado").val(total_selecionado);
        }
    });
}
...
//    if (window.confirm("Confirma a baixa do(s) agendamento(s) selecionado(s)?")) {
...
function estornar_baixa_contas_pagar(bcp_chave_baixa) {
    ...
    var doc = bcp_chave_baixa.substring(14, 28);
    var parcela = bcp_chave_baixa.substring(9, 12);
    
    //if (agendamento!=0){
       // if (window.confirm("ATENCÃO! Esse registro faz parte do agendamento " + agendamento + ", serão estornados todos os registros que foram baixados. Confirma assim mesmo?")) {

            //$.post("estornar_baixa_contas_pagar.php", {bcp_chave: bcp_chave_baixa}, function (get_retorno) {
               // if (get_retorno == 0) {
                //    $("#mensagem_retorno").modal();
