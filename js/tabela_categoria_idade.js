...
$(document).ready(function(){
    $('#tabela_categoria_idade').DataTable({
        responsive: true,
        paging: false,
        ordering: true,
        info: true,
...
$('#modal_editar').on('show.bs.modal', function (event) {
    ...
    var recipientcat_de = button.data('whatevercat_de')
    var recipientcat_ate = button.data('whatevercat_ate')
    var modal = $(this)
    //modal.find('.modal-title').text('Edição da raça - código: ' + recipient)
    //modal.find('.modal-title').text('Edição da raça')
    modal.find('#codigo_categoria').val(recipient)
    modal.find('#categoria_de').val(recipientcat_de)
    modal.find('#categoria_ate').val(recipientcat_ate)
})
...
$('#modal_excluir').on('show.bs.modal', function (event) {
    ...
    var modal = $(this)
    if (tipo_gravacao==3){
        modal.find('.modal-title').text('Categorias - Remover da lixeira ')
    }
    else if (tipo_gravacao==2){
        modal.find('.modal-title').text('Categorias - Enviar para lixeira ')
    }
    //modal.find('.modal-title').text('Edição da raça - código: ' + recipient)
   // modal.find('.modal-title').text('Edição da raça')
    modal.find('#codigo_categoria').val(recipient)
...
$('#modal_incluir').on('show.bs.modal', function (event) {
   ...
   //  modal.find('#nome_bancos').val(recipientcat_de)
})
