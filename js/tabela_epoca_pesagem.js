...
$(document).ready(function(){
    $('.gravar').on('click', function() {
        var formID = document.getElementById("gravar_epoca");
    ...
    $('#tabela_epoca_pesagem').DataTable({
        responsive: true,
        paging: false,
        ordering: true,
        info: true,
...
$('#modal_editar').on('show.bs.modal', function (event) {
    ...
    modal.find('#codigo_epoca_pesagem').val(recipient)
    modal.find('#descricao_epoca_pesagem').val(recipientnome)
})
...
$('#modal_excluir').on('show.bs.modal', function (event) {
    ...
    var modal = $(this)
    if (tipo_gravacao==3){
        modal.find('.modal-title').text('Motivo de Pesagem - Remover da lixeira ')
    }
    else if (tipo_gravacao==2){
        modal.find('.modal-title').text('Motivo de Pesagem - Enviar para lixeira ')
    }
    //modal.find('.modal-title').text('Edição da raça - código: ' + recipient)
   // modal.find('.modal-title').text('Edição da raça')
    modal.find('#codigo_epoca_pesagem').val(recipient)
