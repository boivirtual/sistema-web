...
$(document).ready(function(){
    ...
    $('#tabela_situacao_reprodutiva').DataTable({
        "responsive": true,
        "paging":   false,
        "ordering": false,
        "info":     true,
    ...
    var ativo = $("input[name='ativo_filtro']:checked").val();
...
function listar_estacoes(){
    $('.estacoes').show();
    $('.botao_estacoes').hide();
}

function listar_partos() {
    $('.partos').show();
    $('.botao_partos').hide();
}

function listar_abortos() {
    $('.abortos').show();
    $('.botao_abortos').hide();
}

function fechar_lista_estacoes(){
    $('.estacoes').hide();
    $('.botao_estacoes').show();
}

function fechar_lista_partos(){
    $('.partos').hide();
    $('.botao_partos').show();
}

function fechar_lista_abortos(){
    $('.abortos').hide();
    $('.botao_abortos').show();
}
...
function finalizar_sair() {
    location.href='form_rel_situacao_reprodutiva.php';
}
...
function listar_animais(opcao){
    ...
    if (opcao=='1') {
        if (tipo_rel=='I') {
            location.href='form_situacao_reprodutiva_individual_rel.php?codigo_alfa_numerico=' + codigo_alfa_numerico;
        }
        else {
            location.href='form_situacao_reprodutiva_geral_rel.php?descricao_filtro=' + descricao_filtro + 
            '&tipo_rel=' + tipo_rel + 
            '&local=' + array_local + 
            '&origem=' + array_origem + 
        ...
        if (tipo_rel=='I') {
            location.href='rel_situacao_reprodutiva_individual_excel.php?codigo_alfa_numerico=' + codigo_alfa_numerico;
            limpar_filtros();
            tout = setTimeout('limpar_tela()', 2000);
        }
        else {
            location.href='rel_situacao_reprodutiva_geral_excel.php?descricao_filtro=' + descricao_filtro + 
            '&tipo_rel=' + tipo_rel + 
            '&local=' + array_local + 
            '&origem=' + array_origem + 
            '&categoria=' + array_categoria + 
...
function situacao_reprodutiva_individual_excel() {
    var codigo_alfa_numerico = $("#codigo_number_filtro").val();
...
function situacao_reprodutiva_geral_excel(){
...
function voltar_filtro() {
    ...
    location.href='form_rel_situacao_reprodutiva.php?tipo='+origem_relatorio;
}
...
function voltar_relatorios() {
    ...
    if (origem_relatorio==1) {
        ...
        location.href='form_selecao_matrizes.php?tipo='+origem_relatorio;
    }
}
...
jQuery.extend( jQuery.fn.dataTableExt.oSort, {
    "date-br-pre": function ( a ) {
        if (a == null || a == "") { // Trata valores nulos ou vazios
            return 0; // Ou um valor que você considere adequado para ordenação
        }
        var partes = a.split('/');
        return (partes[2] + partes[1] + partes[0]) * 1;
    },
