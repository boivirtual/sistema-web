...
$(document).ready(function(){
    $('.gravar').on('click', function() {
        var formID = document.getElementById("gravar_causa");
...
$('#modal_editar').on('show.bs.modal', function (event) {
    ...
    modal.find('#codigo_causa_morte').val(recipient)
    modal.find('#descricao_causa_morte').val(recipientnome)
})
...
$('#modal_excluir').on('show.bs.modal', function (event) {
    ...
    var modal = $(this)
    if (tipo_gravacao==3){
        modal.find('.modal-title').text('Causa da Morte - Remover da lixeira ')
    }
    else if (tipo_gravacao==2){
        modal.find('.modal-title').text('Causa da Morte - Enviar para lixeira ')
    }
    //modal.find('.modal-title').text('Edição da raça - código: ' + recipient)
   // modal.find('.modal-title').text('Edição da raça')
    modal.find('#codigo_causa_morte').val(recipient)
