...
function ler_animal_mae(){
    ...
    $.post("ler_animal_femea_nascimento.php", {id_animal:id_animal, local:local, data_nascimento:data_nascimento}, function(valor){
        ...
        if (php[0]=='Nao tem animal') {
            ...
            if (php[8] == php[14]) { // negativo na estacao atual 
                if ((grupo_usuario==01 || grupo_usuario==03)) {
                    $('#modal_estacao .modal-body .desc_modal').html('Diagnóstico negativo estação ' + php[12]); 
                    $('#modal_estacao .modal-body .desc_modal_1').html('-Clique em ALTERAR DIAGNÓSTICO OU;');
            ...
            else { // negativo na estacao anterior e não entrou na estacao atual
                if ((grupo_usuario==01 || grupo_usuario==03)) {
                    $('#modal_estacao .modal-body .desc_modal').html('Atenção! Esta Fêmea não está em estação de monta. Diagnóstico negativo estação ' + php[12]); 
                    $('#modal_estacao .modal-body .desc_modal_1').html('-Clique em ALTERAR DIAGNÓSTICO OU;');
                    $('#modal_estacao .modal-body .desc_modal_2').html('-Informe nova estação para esse nascimento OU;'); 
                    $('#modal_estacao .modal-body .desc_modal_3').html('-Selecione outra fêmea.'); 
                    $('.mens_administrador').hide(); 
                    $('.estacao_monta').show();
                    $('#modal_estacao .outra_estacao').show(); 
                    $('#modal_estacao .alterar_diagnostico').show(); 
                    $('#modal_estacao .outra_femea').show(); 
                    $('#modal_estacao .voltar').hide(); 
                    $('#modal_estacao .fechar').hide(); 
        ...
        if (opcao_nascimento=='N') {
            if (php[4]==0) {
                if ((grupo_usuario==01 || grupo_usuario==03)) {
                    $('#modal_estacao .modal-body .desc_modal').html('Atenção! Esta Fêmea não está em estação de monta.'); 
                    $('#modal_estacao .modal-body .desc_modal_1').html('-Informe nova estação para esse nascimento OU;');
                    $('#modal_estacao .modal-body .desc_modal_2').html('-Selecione outra fêmea.'); 
                    $('#modal_estacao .modal-body .desc_modal_3').html(''); 
                    $('.mens_administrador').hide(); 
                    $('.estacao_monta').show();
            ...
            if (php[10]==1) {
                if ((grupo_usuario==01 || grupo_usuario==03)) {
                    $('#modal_estacao .modal-body .desc_modal').html('Atenção! Esta Fêmea não está em estação de monta. Aborto ou Natimorto estação ' + php[12]); 
                    $('#modal_estacao .modal-body .desc_modal_1').html('-Informe nova Estação para esse nascimento OU;');
                    $('#modal_estacao .modal-body .desc_modal_2').html('-Selecione outra fêmea.'); 
                    $('#modal_estacao .modal-body .desc_modal_3').html(''); 
                    $('.mens_administrador').hide(); 
            ...
            if (php[9]!=0) {
                ...
                if (data_nascimento == php[13]) {
                    ...
                    if ((grupo_usuario==01 || grupo_usuario==03)) {
                        ...
                        $('#modal_estacao .modal-body .desc_modal_2').html('-Informe nova estação para este nascimento OU;'); 
                        $('#modal_estacao .modal-body .desc_modal_3').html('-Selecione outra fêmea.'); 
                        $('.mens_administrador').hide(); 
                        $('.estacao_monta').show();
                        $('#modal_estacao .outra_estacao').show(); 
            ...
            if (php[10]==1) {
                $("#nascimento_aborto_natimorto .modal-body").html('Essa fêmea teve aborto ou natimorto na estação ' + php[12]);
                $('#nascimento_aborto_natimorto').modal('show');
                return;
            }

            if (php[9]!=0) {
                $("#nascimento_aborto_natimorto .modal-body").html('Essa fêmea teve nascimento na estação ' + php[12]);
                $('#nascimento_aborto_natimorto').modal('show');
                return;
            }
        }
...
function alterardiagnostico() {
    ...
    location.href='form_cobertura_animais_femeas_servidas.php?local=' + local + 
    ...
    if (tipo_gravacao==2) {
        ...
        $.ajax({
            ...
            success: function(data){
                ...
                $('.confirma_gravar').html('Confirmar Inclusão');
...
function drag(ev, test){
    ...
    if (test == null){
        ...
        var teste = test.getElementsByTagName('strong')[0].innerHTML;
    }  
    ev.dataTransfer.setData("nome", teste);
}
...
function confirma_nutricao(){
    if($("#dataNutricao").val() != '' && $("#nomeProduto").val() != '000000000' && $("#qtdProduto").val() != '' && $("#undProduto").val() != ''){
        var qtdProduto = $("#qtdProduto").val();
