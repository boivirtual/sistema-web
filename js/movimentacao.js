...
window.addEventListener("load", function(event) {
    consulta_movimentacao_sem_finalizar();    

    var movEditar = $("#editarMovimentacao").val();

    if (movEditar) {
        var local = $("#local_origem").val();
        $.post("lista_animais_pasto_transferencia.php",{local:local,}, 
            function(valor){ 
            var php = valor.split("<|>");
...
function consulta_movimentacao_sem_finalizar() {
    $.post(
        "form_lista_movimentacao_sem_finalizar.php",
        {},
        function (valor) {
            $("div[id=lista_movimentacao_sem_finalizar]").html(valor);
                var tem_pesagem = $(".tem_movimentacao").val();

                if (tem_pesagem=='S') {
                    $("#modal_movimentacao_sem_finalizar").modal();
                }
            }
        );
}
...
// Marca ou Desmarca um animal como selecionado ou não no banco de dados automaticamente 
// ao clicar um de cada vez na lista da consulta dos programas:
// form_movimentacao_animais_incluir.php e form_movimentacao_animais_editar.php
$(document).ready(function() {
    $(document).on('change', '.animalSelecionado', function() {
        // 1. Identifica a linha (tr) do checkbox que foi clicado
        let linha = $(this).closest('tr');
            
        // 2. Captura os dados apenas desta linha
        let animalId = linha.find('.animal_id').text().trim();
        let animalSelecionado = $(this).is(':checked') ? 'S' : 'N';
        let femeaSelecionadaReproducao = linha.find('.femea_selecionada').text().trim();
        let controleReproducao = linha.find('.controle').text().trim();

        if (animalSelecionado=='S') {
            if (femeaSelecionadaReproducao=='S') {
                if (controleReproducao=='C') {
        ...
        //const selecionados = document.querySelectorAll('.animalSelecionado:checked');
        //$('.total_digitados').text('Animais Selecionados: ' + selecionados.length);
        //$('.total_digitados').val(selecionados.length);

        let totalSelecionados = $('#tabela_itens_digitados').DataTable()
            .column(0) // Ajuste o índice para a coluna onde está o checkbox
            .nodes()
            .to$()
            .find(':checked').length;

        $('.total_digitados').text('Animais Selecionados: ' + totalSelecionados);
        $('.total_digitados').val(totalSelecionados);

        let dadosParaEnviar = {
            'animalId': animalId,
            'animalSelecionado': animalSelecionado,
            'idMovimentacaoGravada': $("#idMovimentacaoGravada").val(),
            'flagSelecionados' : 1,
        };

        // Efeito visual na linha que disparou o evento
        linha.css('opacity', '0.5');

        $.ajax({
            url: 'gravar_movimentacao_digitada.php',
            type: 'POST',
            data: dadosParaEnviar, // O jQuery enviará isso como um POST estruturado
            dataType: 'json',
            success: function(response) {
                if (response.status === 'sucesso') {
                    linha.css('opacity', '1');
                } else {
                    tratarErroGravacao(linha, response.mensagem);
                }
            }
        });
    });
...
// Marca ou Desmarca todos os animais como selecionados ou não no banco de dados
// ao selecionar todos na lista da consulta dos programas:
// form_movimentacao_animais_incluir.php e form_movimentacao_animais_editar.php
function selecionarMovimentacaoToda() {
    var table = $('#tabela_itens_digitados').DataTable();
    let listaAnimais = [];

    // Em vez de percorrer o HTML ($), percorremos os NODES do DataTables
    // Isso pega TUDO, mesmo o que está escondido pelo filtro
    table.rows().nodes().to$().each(function() {
        let $linha = $(this);
        let $checkbox = $linha.find('.animalSelecionado');
        
        let animalId = $linha.find('.animal_id').text().trim();
        let flag = $checkbox.is(':checked') ? 'S' : 'N';
...
// Função para reverter a interface em caso de erro
function tratarErroGravacao(linha, mensagem) {
    linha.prop('checked', !linha.is(':checked'));
    
    alert("Atenção: " + mensagem);
}

/*$(document).ready(function() {
    $(document).on('change', '.seleciona_todos', function() {
        
        let $checkboxAtual = $(this);
        let $linha = $checkboxAtual.closest('tr');
        let idsSelecionados = [];

        // Coleta todos os IDs marcados
        $('.animalSelecionado:checked').each(function() {
            let id = $(this).closest('tr').find('.animal_id').text().trim();
            idsSelecionados.push(id);
        });

        // Efeito Visual 1: Deixa a linha levemente transparente para indicar "processando"
        $linha.css('opacity', '0.5');

        var local_origem = $("#local_origem").val();
        var local_destino = $("#local_destino").val();
        var data_movimentacao = $("#data_movimentacao").val();
        var descricao_filtro_dig = $("#descricao_filtro_dig").val();
        var idMovimentacaoGravada = $("#idMovimentacaoGravada").val();
        var tipo_movimentacao = $("input[name='tipo_movimentacao']:checked").val();
        ...
        $.ajax({
            ...
            type: 'POST',
            data: {
                'animais_ids':idsSelecionados,
                'local_origem': local_origem,
                'local_destino': local_destino,
                'data_movimentacao': data_movimentacao,
                'descricao_filtro_dig': descricao_filtro_dig,
                'idMovimentacaoGravada': idMovimentacaoGravada,
                'tipo_movimentacao': tipo_movimentacao
                },
            dataType: 'json', // Adicione isso para ele converter o JSON sozinho
            success: function(response) {
                if (response.status === 'sucesso') {
                    $linha.css('opacity', '1');
                    $("#idMovimentacaoGravada").val(response.idMovimentacao);
                } else {
                    tratarErroGravacao($checkboxAtual, $linha, response.mensagem);
                }
            },
            error: function() {
                tratarErroGravacao($checkboxAtual, $linha, response.mensagem);
...
$(document).ready(function(){
    ...
    $('#pesagem').change(function(event) {
        ...
        $.post("ler_pesagem.php", {id:codigo_pesagem}, function(valor){
            ...
            if (php[0]==999999999) {
                ...
                $("#mensagem_erro").modal();
                $("#mensagem_erro .modal-body").html('ATENÇÃO: Transfira animais machos das seguintes categorias para a Saída:<br>' + php[1]);

                //alert ('ATENÇÃO: Transfira animais machos das seguintes categorias para a Saída: ' + php[1]);
                $("#pesagem").val('000000000');
                return;
            }
            else if (php[2]=='S') {
                $("#mensagem_erro").modal();
                $("#mensagem_erro .modal-body").html('ATENÇÃO: Transfira animais fêmeas das seguintes categorias para a Saída:<br>' + php[3]);
                //alert ('ATENÇÃO: Transfira animais fêmeas das seguintes categorias para a Saída: ' + php[3]);
                $("#pesagem").val('000000000');
                return;
            }
            else {
    ...
    $('#pasto_morte').click(function(event) {
            ...
            $.post("ler_animal_categoria_pasto.php", {local:local, sexo:sexo, codigo_categoria:codigo_categoria, pasto:pasto}, function(valor){
                ...
                if (php_pasto[0]=='N') {
                    if (php_pasto[3]=='') {
                        ...
                        $(".alert_erro_animal .negrito").html('');
                        $(".alert_erro_animal span").html('Não consta no pasto selecionado animais com a categoria ' + desc_categoria + ' ' + sexo_morte);
                        $(".alert_erro_animal").show();
                        $("#pasto_morte").val('000000000');
                        document.getElementById("pasto_morte").focus();
                        return;
...
/*        $.post("lista_categoria_pasto_morte_outra.php", {local:local, pasto:pasto}, function(valor){
    ...
    $('#pasto_outra').click(function(event) {
        $(".alert_erro_animal .negrito").html('');
        $(".alert_erro_animal span").html('');
        $(".alert_erro_animal").hide();
        ...
        if (animal_codigo_id==0 && controle_estoque=='I'){
            ...
            document.getElementById("id_animal_outra").style.borderColor = "red";
            $("#qualModal").val('O');
            return;
        }
    }); 

    $('#observacao_outra').click(function(event) {
        var controle_estoque = $("#controle_estoque").val();
        var animal_codigo_id = $("#codigo_id_outra").val();
...
function voltar_movimentacao_selecionar_digitados() {
    if (verificarSelecaoNaTabela()) {
        ...
        $("#mensagem_sair_sem_confirmar .modal-body .desc_modal").html('Exitem animais selecionados, ao confirmar, a digitação será perdida.');
    } else {
...
function confirmaSairSemGravarMovimentacao() {
    const checks = document.querySelectorAll('input.animalSelecionado');
    
    checks.forEach(check => {
        check.checked = false;
    });

    $("#mensagem_sair_sem_confirmar").modal('hide');
    $("#itens_digitados").hide();
    $("#dados_consulta").show();
    ...
    $.post("lista_animais_transferencia.php", {
        ...
        function(valor){ 
            ...
            if (valor.status === 'error') {
                ...
                var partes = valor.split("SEP_ID");
                var idMovimentacao = partes[0]; // Aqui está o seu código numérico
                var htmlTabela = partes[1]; // Aqui está sua tabela

                var controle_estoque=$("#controle_estoque").val();

                if (controle_estoque=='I') {
                    $("div[id=itens_listados]").html(htmlTabela);
                    $("#idMovimentacaoGravada").val(idMovimentacao);
...
function editar_movimentacao(id_editar){
    // O modal aqui raramente aparece porque o navegador sai da página atual imediatamente
    window.location.href = "form_movimentacao_animais_editar.php?id=" + id_editar;
}

function excluir_movimentacao(id_excluir, tipo_registro){
    $('#id_excluir').val(id_excluir);
    $('#tipo_registro').val(tipo_registro);

    $("#confirma_excluir").modal();
    $("#confirma_excluir .modal-body").html('Confirma excluir esse registro?');
}
...
function confirmar_excluir_movimentacao(){
    ...
    // tipo = 1 ajusta os estoques pois a movimentacao foi confirmada
    // tipo = 2 excluir apenas as movimentacoes pois não teve baixa no estoque
    if (tipo_registro==1) {
        $.ajax({
            type: "POST", 
        ...
        $.ajax({
            type: "POST", 
            url: 'excluir_movimentacao_animais_sem_finalizar.php',
            data: {
                    id_excluir: id_excluir
                },
            success: function(data){
...
function gravar_movimentacao_venda_transferencia() {
    ...
    // Em 08/01/2026 o programa gravar_movimentacao_individual_digitada passou a ser
    // gravar_movimentacao_baixa_venda_transferencia.php e
    // gravar_movimentacao_baixa_venda_transferencia_lote.php  
