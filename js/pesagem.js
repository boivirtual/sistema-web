...
var indiceLinhaRetornoPesagem = -1;
var deveReposicionarLinhaPesagem = false;

function guardarLinhaAtualPesagem(elemento, tipo) {
    var linha = $(elemento).closest("tr");

    if (linha.hasClass("child")) {
        linha = linha.prev();
    }

    var tabela = $("#tabela_itens").DataTable();

    indiceLinhaRetornoPesagem = tabela.row(linha).index();

    deveReposicionarLinhaPesagem = true;
}
...
var configDataTable = {
    ...
    ordering: false,
    info: false,
    language: { sSearch: "Busca:", zeroRecords: "Nada encontrado" },
    ...
    columnDefs: [
        // coluna do checkbox
        { className: "text-center coluna_selecao_motivo", targets: [0] },

        // colunas principais
        { responsivePriority: 1, targets: [1, 11, 12, 21] },

        // controle de expansão (+)
        { className: 'dtr-control', targets: 12 },

        // colunas que aparecem só no expand
        { className: 'none', targets: [13, 14, 15, 16, 17, 19] },

        // centralização
        { className: "text-center", targets: [0, 2, 3, 4, 10, 12, 21] }
    ],
    ...
    $.post("ler_pesagem_online.php", { pesagem_id: numero_pesagem_id,}, function (valor) {
        ...
        for (var i = 0; i < php_array_itens.length; i++) {
            ...
            if (itens[0]!=0 && itens[0]!='') {
                ...
                html += "<td class='' style='display:none; text-align:center;'>" +
                        "<input type='checkbox' class='' value='" + codigo_id + "' data-id-animal='" + codigo_id + "'>" +
                        "</td>";
        ...
        //$(".btnEditar").unbind("click").bind("click", modal_editar_item);
        //$(".btnExcluir").unbind("click").bind("click", excluirItemEdicao);        

        $(".btnEditar").unbind("click").bind("click", function (e) {
            e.preventDefault();
            e.stopPropagation();

            guardarLinhaAtualPesagem(this, "editar");
            modal_editar_item.call(this);
        });
...
/* chamada da rotina para enviar registro de pesagem para lixeira com validação de senha */
var dados_exclusao = {}; // Variável global temporária para guardar os dados

function enviar_pesagem_lixeira($id, $local, $epoca, $opcao) {
    // 1. Remove temporariamente o foco do modal principal para não travar o segundo
    $('#modal_pesagem_sem_finalizar').removeAttr('tabindex');
    
    $("#senha_exclusao").val("");
    $("#erro_senha").hide();
    
    dados_exclusao = { id: $id, local: $local, epoca: $epoca };
    $("#texto_confirmacao_exclusao").html("<strong>" + $local + "</strong><br>" + $epoca);
    
    // 2. Abre o modal de senha
    $("#modal_confirma_exclusao").modal('show');
}

// 3. Garante o foco ao abrir e restaura ao fechar
$('#modal_confirma_exclusao').on('shown.bs.modal', function () {
    $('#senha_exclusao').focus();
});

$('#modal_confirma_exclusao').on('hidden.bs.modal', function () {
    // Restaura o atributo do modal principal ao fechar o de senha
    $('#modal_pesagem_sem_finalizar').attr('tabindex', '-1');
});

// Evento de clique no OK do Modal
$("#btn_confirmar_exclusao_modal").on('click', function() {
    var senhaDigitada = $("#senha_exclusao").val();
    var senhaCorreta = senhaUsuarioLogado;

    if (senhaDigitada === senhaCorreta) {
        $("#modal_confirma_exclusao").modal('hide');
        
        // EXECUTA O SEU AJAX ORIGINAL
        $.post("excluir_pesagem.php", { id: dados_exclusao.id }, function (valor) {
            var php = valor.split("<|>");
            if (php[0] == 9) {
                $("#mensagem_erro").modal();
                ...
                // Opcional: recarregar após fechar o retorno
            }
        });
    } else {
        $("#erro_senha").fadeIn();
        $("#senha_exclusao").val("").focus();
    }
});
...
function consulta_pesagem_sem_finalizar() {
    /*var controle_estoque = $("#controle_estoque").val();

    if (controle_estoque == "I") {*/
        $.post(
            "form_lista_pesagem_individual_sem_finalizar.php",
            {},
            function (valor) {
                $("div[id=lista_pesagem_sem_finalizar]").html(valor);

                var tem_pesagem = $(".tem_pesagem").val();

                if (tem_pesagem=='S') {
                    $("#modal_pesagem_sem_finalizar").modal();
                }
            }
        );
    //}
...
// programa pesagem,  modal_pesagem_sem_finalizar
function selecionar_fazendas(elemento) {
...
function Salvar() {
    ...
    var novaLinha = t.row.add([
        ...
        $("#observacao").val(),                         // 7
        $("#codigo_id").val(),                          // 8
        $("#ultimo_peso_animal").val(),                 // 9
        "<div class='btn-group btnEditar'><a class='btn' href='#'><i class='icon_pencil' title='Editar'></i></a></div><div class='btn-group btnExcluir'><a class='btn' href='#'><i class='icon_trash_alt' title='Excluir'></i></a></div>" // 10
...
function excluirItemEdicao() {
    ...
    if (window.confirm("Confirma enviar esse registro para lixeira? " + tdCodigo.html())) {
        guardarLinhaAtualPesagem(this, "excluir");
...
function reposicionarLinhaPesagemAgora() {
    if (deveReposicionarLinhaPesagem && indiceLinhaRetornoPesagem >= 0) {
        var linhas = $("#tabela_itens tbody tr").filter(function () {
            return $(this).find(".id_animal").length > 0;
        });

        var indice = indiceLinhaRetornoPesagem;

        if (indice >= linhas.length) {
            indice = linhas.length - 1;
        }

        var linhaEncontrada = linhas.eq(indice);

        if (linhaEncontrada.length) {
            linhaEncontrada[0].scrollIntoView({
                behavior: "auto",
                block: "center"
            });

            linhaEncontrada.addClass("linha_retorno_pesagem");

            setTimeout(function () {
                linhaEncontrada.removeClass("linha_retorno_pesagem");
            }, 2000);
        }
    }
}
