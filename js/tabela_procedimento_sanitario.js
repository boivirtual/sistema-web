...
$(document).ready(function(){
    $('#tabela_procedimento_sanitario').DataTable({
        "paging":   false,
        "ordering": true,
        "info":     true,
        "language": {
    ...
    $('.gravar').on('click', function() {
        var formID = document.getElementById("gravar_procedimento");
...
$('#modal_editar').on('show.bs.modal', function (event) {
    ...
    modal.find('#codigo_procedimento_sanitario').val(recipient)
    modal.find('#descricao_procedimento_sanitario').val(recipientnome)
})
...
$('#modal_excluir').on('show.bs.modal', function (event) {
    ...
    var modal = $(this)
    if (tipo_gravacao==3){
        modal.find('.modal-title').text('Procedimento Sanitário - Remover da lixeira ')
    }
    else if (tipo_gravacao==2){
        modal.find('.modal-title').text('Procedimento Sanitário - Enviar para lixeira ')
    }
    //modal.find('.modal-title').text('Edição da raça - código: ' + recipient)
   // modal.find('.modal-title').text('Edição da raça')
    modal.find('#codigo_procedimento_sanitario').val(recipient)
