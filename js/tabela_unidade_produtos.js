...
$(document).ready(function(){
    $('#tabela_unidade_produtos').DataTable({
        responsive: true,
        paging: false,
        ordering: true,
        info: true,
    ...
    $('.gravar').on('click', function() {
        var formID = document.getElementById("gravar_unidade");
...
$('#modal_editar').on('show.bs.modal', function (event) {
    ...
    modal.find('#codigo_unidade_produtos').val(recipient)
    modal.find('#descricao_unidade_produtos').val(recipientnome)
})
...
$('#modal_excluir').on('show.bs.modal', function (event) {
    ...
    var modal = $(this)
    if (tipo_gravacao==3){
        modal.find('.modal-title').text('Unidade de Produtos - Remover da lixeira ')
    }
    else if (tipo_gravacao==2){
        modal.find('.modal-title').text('Unidade de Produtos - Enviar para lixeira ')
    }
    //modal.find('.modal-title').text('Edição da raça - código: ' + recipient)
   // modal.find('.modal-title').text('Edição da raça')
    modal.find('#codigo_unidade_produtos').val(recipient)
