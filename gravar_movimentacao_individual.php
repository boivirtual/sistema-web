<?php 
	...
	if ($controle_estoque=='I' && $tipo_gravacao==1 && ($codigo_tipo==3 || $codigo_tipo==5)) {
	...
	else if ($controle_estoque=='I' && $tipo_gravacao==1 && ($codigo_tipo==888 || $codigo_tipo==999)) {
		...
		for($i=0; $i < $quantidade_itens; $i++) {
			...
			$sql_pasto = "SELECT * FROM tbl_animal_pasto 
				WHERE tbl_animal_pasto_local = '$local_origem' AND 
				      tbl_animal_pasto_id = '$codigo_pasto' AND 
				      tbl_animal_pasto_sexo = '$sexo' AND 
				      tbl_animal_pasto_nascimento = '$data' AND 
				  	 (tbl_animal_pasto_marcado_baixar='' OR 
				   	  tbl_animal_pasto_marcado_baixar IS NULL)";
			$tbl_pasto = mysqli_query($conector, $sql_pasto);
		        ...
		        if ($num_rows_pasto!=0) {
 					while ($reg_pasto = mysqli_fetch_object($tbl_pasto)){
				        ...
				        for($j = 0; $j < count($arrayCategorias); $j++){
				            ...
				            if ($idade_atual >= $idade_de && $idade_atual <= $idade_ate) {
				                $codigo_categoria_atual = $id_categoria;

				                if ($idade_ate==999999999) {
				                    $desc_categoria_atual=' > 36 meses';
				                }
				                else {
				                    $desc_categoria_atual= $idade_de . ' a ' . $idade_ate . ' meses';
				                }
				            }
				        } 
	...
	if ($tipo_gravacao==1 && 
		...
		for($i=0; $i < $quantidade_itens; $i++) {
			...
			if ($controle_estoque=='L') {
				// Para controle de estoque por Lote
				$tbl_pasto = mysqli_query($conector, "SELECT * FROM tbl_animal_pasto 
					WHERE tbl_animal_pasto_local = '$local_origem' AND 
					      tbl_animal_pasto_id = '$codigo_pasto' AND 
					      tbl_animal_pasto_sexo = '$sexo' AND 
		        ...
		        if ($num_rows_pasto==0) {
				 	header('Content-type: application/json');
				   	echo json_encode(array('error' => true, 'message' => 'Não existe animais com o sexo ' .$sexo . ' categoria ' .$desc_categoria. ' no pasto. 5'));
			...
			/*if ($controle_estoque=='I') {
				...
				// primeiro marca todos os animais com data nascimento/sexo que ja estao no pasto
				$tbl_pasto = mysqli_query($conector, "SELECT * FROM tbl_animal_pasto 
					WHERE tbl_animal_pasto_local = '$local_origem' AND 
					      tbl_animal_pasto_id = '$codigo_pasto' AND 
					      tbl_animal_pasto_sexo = '$sexo' AND 
			...
			else { // Para controle de estoque por Lote
				$tbl_pasto = mysqli_query($conector, "SELECT * FROM tbl_animal_pasto 
					WHERE tbl_animal_pasto_local = '$local_origem' AND 
					      tbl_animal_pasto_id = '$codigo_pasto' AND 
					      tbl_animal_pasto_sexo = '$sexo' AND 
